...
credit_train <- credit[1,2][rand_vector < 0.9,]
...
#rpart package
library('rpart')
formula<- paste(names(credit_train)[17],
                paste(names(credit_train)[-17], collapse = ' + '),
                sep = ' ~ ')
print(formula)
credit_model_rpart <- rpart(formula, data = credit_train)


par(cex = 0.5)
plot(credit_model_rpart)
text(credit_model_rpart)

print(calcAUC(predict(credit_model_rpart, newdata = credit_train)[,2], credit_train$default))
print(calcAUC(predict(credit_model_rpart, newdata = credit_test)[,2], credit_test$default))


#rpart.control
credit_model_rpart_control <- rpart(formula, data = credit_train,
                                    control = rpart.control(cp=0.01,
                                                            minsplit = 50,
                                                            mindbucket = 20,
                                                            maxdepth = 5))
credit_model_rpart_control
print(calcAUC(predict(credit_model_rpart_control, newdata = credit_train)[,2], credit_train$default))
print(calcAUC(predict(credit_model_rpart_control, newdata = credit_test)[,2], credit_test$default))
